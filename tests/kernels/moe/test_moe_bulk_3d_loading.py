...
# Ensure repo root is prioritized in sys.path when executed directly
...
def test_bulk_3d_vs_unbind_equivalence(tp_rank: int, is_transposed: bool):
    ...
    print(f"PASS (TP): tp_rank={tp_rank}, is_transposed={is_transposed} - exact bitwise match!")
...
def test_bulk_3d_with_linear_ep(ep_rank: int, tp_rank: int):
    ...
    assert torch.equal(layer_bulk.w13_weight, layer_unbind.w13_weight)
    assert torch.equal(layer_bulk.w2_weight, layer_unbind.w2_weight)
    print(f"PASS (TP+EP): ep_rank={ep_rank}, tp_rank={tp_rank} - exact bitwise match!")
...
def test_online_2d_buffering_equivalence(tp_rank: int, arrival_order: str):
    ...
    print(f"PASS (Online 2D): tp_rank={tp_rank}, order={arrival_order} - exact bitwise match!")
...
def test_online_2d_buffering_with_linear_ep(ep_rank: int, tp_rank: int):
    ...
    assert torch.equal(layer_online.w13_weight, layer_unbind.w13_weight)
    assert torch.equal(layer_online.w2_weight, layer_unbind.w2_weight)
    print(f"PASS (Online 2D+EP): ep_rank={ep_rank}, tp_rank={tp_rank} - exact bitwise match!")
...
def test_online_2d_buffering_partial_flush():
    ...
    assert torch.equal(layer_online.w13_weight, layer_unbind.w13_weight)
    assert torch.equal(layer_online.w2_weight, layer_unbind.w2_weight)
    print("PASS (Partial Flush): residual drain matches unbind exactly!")
...
def test_online_2d_per_expert_fused_w13():
    ...
    assert torch.equal(layer_online.w13_weight, layer_unbind.w13_weight)
    assert torch.equal(layer_online.w2_weight, layer_unbind.w2_weight)
    print("PASS (Per-Expert Fused w13): online buffering matches unbind exactly!")
...
def test_online_2d_buffering_on_cuda_gpu():
    ...
    print("PASS (CUDA GPU): exact bitwise match with CPU reference!")
...
if __name__ == "__main__":
    ...
    print("\nALL BITWISE TESTS PASSED (BULK 3D + ONLINE 2D STAGING + RESIDUAL DRAIN + CUDA GPU)!")
